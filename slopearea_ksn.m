...
% slope-area relation of a stream network (MODIFIED FOR KSN)
%
% Syntax
%
...
% ENFORCE -.45 theta value for ksn

% fit power law S = k*A^(-.45)
switch fitmethod
    case 'ls'
        beta = fminsearch(@(beta) sum((g - beta(1)*a.^-.45).^2),beta0);
    case 'lad'
        beta = fminsearch(@(beta) sum(abs(g - beta(1)*a.^-.45)),beta0);
end
